...
from coreapp.serializers import RestaurantSerializer, MealtSerializer, \
OrderSerializer, OrderStatusSerializer, OrderDriverSerializer
...
def customer_add_order(request):
    ...
    if request.method == "POST":
        ...
        # Check if meals in only one restaurant and then calc the order total
        order_total = 0
...
def driver_pick_order(request):
    if request.method == "POST":
        ...
        if Order.objects.filter(driver=driver).exclude(status=Order.ONTHEWAY):
            return JsonResponse({
                "status": "failed",
                "error": "You outstanding order is not delivered yet."
            })
